...
#include "open_spiel/games/orbital_velocity.h"
...
const GameType kGameType{
    /*short_name=*/"orbital_velocity",
    /*long_name=*/"Orbital Velocity Game",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new OrbitalVelocityGame(params));
}

REGISTER_SPIEL_GAME(kGameType, Factory);

}  // namespace
OrbitalVelocityState::OrbitalVelocityState(std::shared_ptr<const Game> game)
    : State(game),
...
int OrbitalVelocityState::CurrentPlayer() const {
  if (IsTerminal()) {
...
void OrbitalVelocityState::DoApplyAction(Action move) {
  if (IsChanceNode()) {
...
std::vector<Action> OrbitalVelocityState::LegalActions() const {
...
std::string OrbitalVelocityState::ActionToString(Player player, Action move) const {
  if (player == kChancePlayerId)
    ...
    SpielFatalError(
        absl::StrCat("Error in OrbitalVelocityState::ActionToString(). Available actions "
                     "are 0, 1, 2, not ",
                     move));
}

std::string OrbitalVelocityState::ToString() const {
  std::string result;
...
bool OrbitalVelocityState::IsTerminal() const {
  return remaining_players_ == 1 || (round_ == 2 && ReadyForNextRound());
}

std::vector<double> OrbitalVelocityState::Returns() const {
  if (!IsTerminal()) {
...
std::string OrbitalVelocityState::InformationState(Player player) const {
...
std::string OrbitalVelocityState::Observation(Player player) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
  std::string result;
...
void OrbitalVelocityState::InformationStateAsNormalizedVector(
...
void OrbitalVelocityState::ObservationAsNormalizedVector(
    Player player, std::vector<double>* values) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
...
std::unique_ptr<State> OrbitalVelocityState::Clone() const {
  return std::unique_ptr<State>(new OrbitalVelocityState(*this));
}

std::vector<std::pair<Action, double>> OrbitalVelocityState::ChanceOutcomes() const {
...
int OrbitalVelocityState::NextPlayer() const {
  ...
  SpielFatalError("Error in OrbitalVelocityState::NextPlayer(), should not get here.");
}

int OrbitalVelocityState::RankHand(Player player) const {
...
void OrbitalVelocityState::ResolveWinner() {
...
void OrbitalVelocityState::SequenceAppendMove(int move) {
  if (round_ == 1) {
...
void OrbitalVelocityState::Ante(Player player, int amount) {
  pot_ += amount;
  ante_[player] += amount;
  money_[player] -= amount;
}

std::vector<int> OrbitalVelocityState::padded_betting_sequence() const {
...
OrbitalVelocityGame::OrbitalVelocityGame(const GameParameters& params)
...
std::unique_ptr<State> OrbitalVelocityGame::NewInitialState() const {
  return std::unique_ptr<State>(new OrbitalVelocityState(shared_from_this()));
}

std::vector<int> OrbitalVelocityGame::InformationStateNormalizedVectorShape() const {
...
std::vector<int> OrbitalVelocityGame::ObservationNormalizedVectorShape() const {
  // One-hot encoding for player number (who is to play).
  // 2 slots of cards (total_cards_ bits each): private card, public card
...
double OrbitalVelocityGame::MaxUtility() const {
  // In orbital Velocity the Maximum utility is the winning the orbit. Represented by 1.
  return 1;
}

double OrbitalVelocityGame::MinUtility() const {
  // In OrbitalVelocity the minimum utility is losing the orbit. This is represented by 0
  return 0;
}
